...
exports.logout = (req, res) => {
  res.cookie('jwt', 'logmeoutyousonofabitch', {
...
exports.protect = catchAsync(async (req, res, next) => {
    ...
    // 4) Chexk if userchanged passwords after jwt was issued
...
// Only for rendered pages hence no errors in this middleware
  ...
  if (req.cookies.jwt) {
    try {
      ...
      const decoded = await promisify(jwt.verify)(
        ...
        // 3) Check if userchanged passwords after jwt was issued
        if (currentUser.changedPasswordAfter(decoded.iat)) {
          return next();
        }
...
exports.resetPassword = catchAsync(async (req, res, next) => {
    ...
    // 4) Log the user in, send the JSON web token in 
    createSendToken(user, 200, res);
});
